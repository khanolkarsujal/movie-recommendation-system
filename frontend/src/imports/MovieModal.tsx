...
interface GenreDotProps {
  genre: string;
}

const GenreDot: React.FC<GenreDotProps> = memo(({ genre }) => (
  <span
...
const MovieModal: React.FC<MovieModalProps> = ({ movie, onClose }) => {
  ...
  const numericRating = typeof movie.rating === 'number' ? movie.rating : 7.5;
  const rColor = ratingColor(numericRating);
        ...
        >
          <div className="relative w-full h-[280px] md:h-[360px] flex-shrink-0 overflow-hidden">
            ...
            <div className="absolute bottom-5 left-6 right-6">
              ...
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <button
                  className="w-10 h-10 rounded-full bg-white/10 border border-white/30 flex items-center justify-center text-white hover:bg-white/20 transition-all"
          ...
          <div className="flex-1 overflow-y-auto p-6 hide-scrollbar">
            <div className="flex items-center gap-3 text-[14px] font-bold text-white mb-6">
              ...
              <span className="border border-white/30 px-1.5 py-0.5 rounded text-white/70 text-[11px]">
                {numericRating >= 8.5 ? 'TV-MA' : 'PG-13'}
              </span>
              ...
              <div className="flex items-center gap-1.5 ml-auto">
                ...
                <span className="font-bold text-white">{numericRating.toFixed(1)}</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              <div className="flex items-center gap-3 text-[13px]">
                ...
                <span className="text-white/90 font-semibold ml-auto">{movie.duration}</span>
              </div>
              <div className="flex items-center gap-3 text-[13px]">
                <Calendar size={14} className="text-white/40 flex-shrink-0" />
                <span className="text-white/40">Year</span>
                <span className="text-white/90 font-semibold ml-auto">{movie.year || 2025}</span>
              </div>
              <div className="flex items-center gap-3 text-[13px]">
                <Tv size={14} className="text-white/40 flex-shrink-0" />
                <span className="text-white/40">Platform</span>
                <span className="font-semibold ml-auto" style={{ color: 'var(--accent)' }}>SLOTHUI ORIGINAL</span>
              </div>
              <div className="flex items-center gap-3 text-[13px]">
                <Star size={14} className="text-white/40 flex-shrink-0" />
                <span className="text-white/40">Rating</span>
                <span className="font-bold ml-auto" style={{ color: rColor }}>{numericRating.toFixed(1)} / 10</span>
              </div>
            </div>
            ...
            {movie.progress !== undefined && movie.progress > 0 && movie.progress < 100 && (
              <div className="mb-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-3">
                {[1, 2, 3].map(i => (
                  <div key={i} className="skeleton rounded-lg" style={{ height: 100 }} />
                ))}
              </div>
            </div>
          </div>
