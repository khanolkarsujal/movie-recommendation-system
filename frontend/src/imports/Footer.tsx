  ...
  return (
    <footer className="border-t border-white/5 bg-[var(--bg-base)] px-6 md:px-[var(--row-padding)] pt-12 pb-8 mt-16">
      <div className="flex items-center gap-4 mb-10">
        {SOCIALS.map(({ icon: Icon, href, label }, i) => (
          <a
            key={i}
            href={href}
            className="w-9 h-9 rounded-full border border-white/10 flex items-center justify-center text-white/40 hover:text-white hover:border-white/30 transition-all hover:-translate-y-0.5 outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]"
            aria-label={label}
          >
            <Icon />
          </a>
            ...
            <ul className="space-y-2.5">
              {items.map((item) => (
                <li key={item}>
                  <a
                    href="#"
                    className="text-[13px] text-white/50 hover:text-white/80 transition-colors outline-none focus-visible:text-white focus-visible:underline"
                  >
                    {item}
                  </a>
                </li>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4">
        <p className="text-[12px] text-white/25">© 2026 CineStream. All rights reserved.</p>
