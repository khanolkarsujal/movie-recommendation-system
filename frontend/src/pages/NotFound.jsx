    ...
    <div className="fixed inset-0 bg-[var(--bg-base)] flex flex-col items-center justify-center text-center px-8 z-50">
      ...
      <div className="relative mb-6 select-none">
        ...
        <motion.h1
          ...
          style={{
            ...
            textShadow: glitch ? '-3px 0 rgba(139,92,246,0.5), 3px 0 rgba(239,68,68,0.5)' : 'none',
          }}
        >
          404
        </motion.h1>
      ...
      >
        ...
        <div className="flex items-center justify-center gap-4 flex-wrap">
          <button
            onClick={() => navigate(-1)}
            className="flex items-center gap-2 px-5 py-2.5 rounded-lg border border-white/15 text-white/70 hover:text-white hover:border-white/30 text-[14px] font-semibold transition-all hover:-translate-y-0.5"
          >
            <ArrowLeft size={16} /> Go Back
          </button>
          <button
            onClick={() => navigate('/browse')}
            className="flex items-center gap-2 px-6 py-2.5 rounded-lg bg-[var(--accent)] text-white text-[14px] font-bold transition-all hover:opacity-90 hover:-translate-y-0.5 shadow-[0_8px_24px_rgba(139,92,246,0.35)]"
          >
      ...
      <div className="absolute inset-0 pointer-events-none"
        style={{ background: 'radial-gradient(ellipse at center 60%, rgba(139,92,246,0.06) 0%, transparent 70%)' }}
      />
    </div>
  );
}
